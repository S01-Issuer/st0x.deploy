...
/// and `LibProdAuthoriser` are hydrated AND the resulting V4 impl + facet are
...
///
/// Once the placeholders are hydrated (`RAIN_VATS_TBD` → real tag, address(0)
...
contract StoxProdV4Test is Test {
    ...
    function checkAllV4OnChain() internal view {
        assertTrue(
            LibProdDeployV4.STOX_RECEIPT_VAULT_RAIN_VATS_TBD.code.length > 0, "V4 StoxReceiptVault impl not deployed"
        );
        assertEq(
            LibProdDeployV4.STOX_RECEIPT_VAULT_RAIN_VATS_TBD.codehash,
            LibProdDeployV4.STOX_RECEIPT_VAULT_CODEHASH_RAIN_VATS_TBD,
            "V4 StoxReceiptVault impl codehash mismatch"
        );

        assertTrue(
            LibProdDeployV4.STOX_CORPORATE_ACTIONS_FACET_RAIN_VATS_TBD.code.length > 0,
            "V4 StoxCorporateActionsFacet not deployed"
        );
        assertEq(
            LibProdDeployV4.STOX_CORPORATE_ACTIONS_FACET_RAIN_VATS_TBD.codehash,
            LibProdDeployV4.STOX_CORPORATE_ACTIONS_FACET_CODEHASH_RAIN_VATS_TBD,
