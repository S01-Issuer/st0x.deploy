...
contract ST0xOrchestratorBeaconSetDeployerTest is Test {
    ...
    /// route to the runtime, then mock `receipt()` to return `receiptAddr`.
    function _prepareVault(address vault_, address receiptAddr) internal {
        // Etch some non-zero code — the actual bytes don't matter because
        // every call into the vault is mocked. Use a single STOP so any
        // un-mocked call cheaply reverts / halts.
        vm.etch(vault_, hex"00");
    ...
    function testFuzzDeploySuccess(
        ...
        // Locate the Deployment event.
        bytes32 depSig = keccak256("Deployment(address,address,address)");
        bool found = false;
        ...
        assertEq(clone.nextBurnReceiptId(), 0, "clone.nextBurnReceiptId()");
    ...
    function testFuzzDeployAnyCaller(
        ...
        vm.expectEmit(true, false, true, false, address(deployer));
        emit ST0xOrchestratorBeaconSetDeployer.Deployment(caller, address(0), vault_);
        vm.prank(caller);
        address orchestrator = deployer.deploy(OffchainAssetReceiptVault(payable(vault_)), owner);
