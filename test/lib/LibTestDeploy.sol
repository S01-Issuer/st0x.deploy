...
library LibTestDeploy {
    function deployWrappedTokenVaultBeaconSet(Vm vm) internal {
        ...
        require(vault == LibProdDeployV4.STOX_WRAPPED_TOKEN_VAULT_CANDIDATE, "StoxWrappedTokenVault address mismatch");

        address beacon = LibRainDeploy.deployZoltu(type(StoxWrappedTokenVaultBeacon).creationCode);
        require(
            beacon == LibProdDeployV4.STOX_WRAPPED_TOKEN_VAULT_BEACON_CANDIDATE,
        ...
        require(
            deployer == LibProdDeployV4.STOX_WRAPPED_TOKEN_VAULT_BEACON_SET_DEPLOYER_CANDIDATE,
    ...
    function deployOffchainAssetReceiptVaultBeaconSet(Vm vm) internal {
        ...
        require(receipt == LibProdDeployV4.STOX_RECEIPT_CANDIDATE, "StoxReceipt address mismatch");

        address receiptVault = LibRainDeploy.deployZoltu(type(StoxReceiptVault).creationCode);
        require(receiptVault == LibProdDeployV4.STOX_RECEIPT_VAULT_CANDIDATE, "StoxReceiptVault address mismatch");

        address deployer = LibRainDeploy.deployZoltu(type(StoxOffchainAssetReceiptVaultBeaconSetDeployer).creationCode);
        require(
            deployer == LibProdDeployV4.STOX_OFFCHAIN_ASSET_RECEIPT_VAULT_BEACON_SET_DEPLOYER_CANDIDATE,
    ...
    function deployAll(Vm vm) internal {
        ...
        require(unified == LibProdDeployV4.STOX_UNIFIED_DEPLOYER_CANDIDATE, "StoxUnifiedDeployer address mismatch");
    }
}
