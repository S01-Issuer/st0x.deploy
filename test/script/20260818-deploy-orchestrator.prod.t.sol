...
/// pending — the 0.1.8 closure or the instance deploy never landed. Run the
...
///
/// 1. **Closure pending** (Ethereum / HyperEVM until
///    `manual-sol-artifacts-0-1-8` ships the audited set): pins the
///    pre-flight refusal — `run()` reverts `ClosureNotDeployed` naming the
///    first missing contract — and logs the outstanding dispatch.
/// 2. **Instance pending** (closure live, no instance — Base today): drives
...
contract DeployOrchestratorProdTest is Test {
    ...
    function assertOrchestratorRollout(string memory label) internal {
        ...
        if (setDeployer.code.length == 0) {
            ...
            console2.log(string.concat("PENDING [", label, "]: 0.1.8 closure not deployed"));
            console2.log("-> dispatch manual-sol-artifacts-0-1-8 for every suite, then 20260818-deploy-orchestrator");
