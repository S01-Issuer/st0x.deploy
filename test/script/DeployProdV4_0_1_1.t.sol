...
import {Deploy} from "../../script/DeployProdV4_0_1_1.sol";
import {LibStoxDeployNetworks, UnknownDeploymentNetwork} from "../../src/lib/LibStoxDeployNetworks.sol";

/// @title DeployProdV4_0_1_1Test
/// @notice Network selection for the 0.1.1 bootstrap deploy script: which
/// networks one dispatch broadcasts to. Nothing here forks or broadcasts —
/// what the selection feeds (`LibRainDeploy.deployToNetworks`) is upstream's.
contract DeployProdV4_0_1_1Test is Test {
    ...
    /// The vetted bootstrap set the workflow's `network` choice mirrors, in the
    /// order the script broadcasts them.
    function testSupportedNetworks() external view {
        string[] memory supported = script.supportedNetworks();
        assertEq(supported.length, 2);
        assertEq(supported[0], LibStoxDeployNetworks.ETHEREUM);
        assertEq(supported[1], LibStoxDeployNetworks.HYPEREVM);
    }
    ...
    /// Base is rejected by design: its production instances were never
    /// bootstrapped from the 0.1.1 set. The 0.1.30 script accepts `base`, so
    /// this is the one difference between the two supported sets.
    function testBaseIsRejected() external {
        vm.expectRevert(abi.encodeWithSelector(UnknownDeploymentNetwork.selector, LibRainDeploy.BASE));
        this.callSelectNetworks(LibRainDeploy.BASE);
    }

    /// `all` must not smuggle Base in behind the named option's back.
    function testAllExcludesBase() external view {
        string[] memory selected = this.callSelectNetworks(LibStoxDeployNetworks.ALL);
        for (uint256 i = 0; i < selected.length; i++) {
            assertTrue(keccak256(bytes(selected[i])) != keccak256(bytes(LibRainDeploy.BASE)));
        }
    }
