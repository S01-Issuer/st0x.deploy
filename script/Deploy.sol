...
bytes32 constant DEPLOYMENT_SUITE_STOX_ST0X_ORCHESTRATOR_BEACON_SET_DEPLOYER_V4 =
    keccak256("stox-st0x-orchestrator-beacon-set-deployer-v4");
...
contract Deploy is Script {
    ...
    function run() public {
        ...
        if (suite == DEPLOYMENT_SUITE_STOX_RECEIPT_V4) {
            ...
            // on nothing at deploy time.
            deploySuite(
                type(ST0xOrchestrator).creationCode,
                "src/concrete/ST0xOrchestrator.sol:ST0xOrchestrator",
                LibProdDeployV4.ST0X_ORCHESTRATOR_RAIN_VATS_0_1_6,
                LibProdDeployV4.ST0X_ORCHESTRATOR_CODEHASH_RAIN_VATS_0_1_6,
                noDeps
            );
        } else if (suite == DEPLOYMENT_SUITE_STOX_ST0X_ORCHESTRATOR_BEACON_SET_DEPLOYER_V4) {
            // Depends on the ST0xOrchestrator impl (embedded via
            // LibProdDeployV4 in the constructor).
            address[] memory deps = new address[](1);
            deps[0] = LibProdDeployV4.ST0X_ORCHESTRATOR_RAIN_VATS_0_1_6;
            deploySuite(
                type(StoxST0xOrchestratorBeaconSetDeployer).creationCode,
                "src/concrete/deploy/StoxST0xOrchestratorBeaconSetDeployer.sol:StoxST0xOrchestratorBeaconSetDeployer",
