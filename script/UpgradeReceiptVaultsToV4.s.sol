...
/// match the pinned `LibProdDeployV4.STOX_RECEIPT_VAULT_CODEHASH_RAIN_VATS_TBD`.
...
/// @notice The V4 authoriser clone constant in `LibProdAuthoriser` is still
...
/// pinned `LibProdAuthoriser.STOX_PROD_AUTHORISER_V4_CLONE_CODEHASH` — the
...
/// in `LibProdAuthoriser.expectedGrants()`. The clone's initialization +
...
/// @notice Forge script that authors the receipt-vault V4 upgrade plus the
/// authoriser swap onto the corporate-action-aware V4 clone. The ST0x token-
/// owner Safe signs a single bundle that:
///
///   1. Points the receipt-vault beacon at the V4 implementation
///      (`LibProdDeployV4.STOX_RECEIPT_VAULT_RAIN_VATS_TBD`).
///   2. Calls `setAuthorizer(LibProdAuthoriser.STOX_PROD_AUTHORISER_V4_CLONE)`
...
/// clone whose role-grant map matches `LibProdAuthoriser.expectedGrants()`.
///
...
///    vault (13 calls). Compute the canonical `SafeTxHash` against the live
...
contract UpgradeReceiptVaultsToV4 is Script {
    ...
    address internal constant V4_IMPL = LibProdDeployV4.STOX_RECEIPT_VAULT_RAIN_VATS_TBD;
    ...
    address internal constant V4_AUTHORISER_CLONE = LibProdAuthoriser.STOX_PROD_AUTHORISER_V4_CLONE;
    ...
    function run() external {
        ...
        if (v4Codehash != LibProdDeployV4.STOX_RECEIPT_VAULT_CODEHASH_RAIN_VATS_TBD) {
            revert V4CodehashMismatch(V4_IMPL, LibProdDeployV4.STOX_RECEIPT_VAULT_CODEHASH_RAIN_VATS_TBD, v4Codehash);
        }
        ...
        if (cloneCodehash != LibProdAuthoriser.STOX_PROD_AUTHORISER_V4_CLONE_CODEHASH) {
            revert V4AuthoriserCloneCodehashMismatch(
                V4_AUTHORISER_CLONE, LibProdAuthoriser.STOX_PROD_AUTHORISER_V4_CLONE_CODEHASH, cloneCodehash
            );
        }
        IAccessControl cloneAcl = IAccessControl(V4_AUTHORISER_CLONE);
        LibProdAuthoriser.RoleGrant[] memory expected = LibProdAuthoriser.expectedGrants();
        ...
        LibBeaconInvariants.assertBeaconInvariants(BEACON, LibProdSafes.STOX_TOKEN_OWNER_SAFE, V4_IMPL);
        ...
        // authoriser via `LibProdTokensBase.PROD_RECEIPT_VAULT_AUTHORISER`,
        ...
        LibSafeInvariants.assertThreshold(safe, LibProdSafes.STOX_TOKEN_OWNER_SAFE_THRESHOLD);
