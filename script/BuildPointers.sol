...
contract BuildPointers is Script {
    ...
    function run() external {
        ...
        // `LibFs.buildFileForTaggedContract` creates the tag dir as needed.
    ...
    // set per release tag, each aliasing that tag's frozen pointer-file
    ...
    /// @notice Pointer contract names in a fixed order.
    ...
    /// @notice The pointer file name for `(tag, name)`. rain-sol-codegen
    /// 0.1.36's `LibFs` writes `<name>.sol`, so that is the name wherever it
    /// exists on disk; snapshots frozen before this repo adopted that scheme
    /// keep the `<name>.pointers.sol` name they were cut with, forever.
    function pointerFile(string memory tag, string memory name) internal returns (string memory) {
        string memory plain = string.concat(name, ".sol");
        if (vm.exists(string.concat("src/generated/", tag, "/", plain))) {
            return plain;
        }
        return string.concat(name, ".pointers.sol");
    }

    function pointerExists(string memory tag, string memory name) internal returns (bool) {
        return vm.exists(string.concat("src/generated/", tag, "/", pointerFile(tag, name)));
    }
    ...
    function v4ImportLine(string memory fileName, string memory base, string memory tag)
        internal
        pure
        returns (string memory)
    {
        ...
        string memory tail = string.concat("_RUNTIME_", suffix, '_GEN} from "./', tag, "/", fileName, '";');
        ...
        for (uint256 t = 0; t < tags.length; t++) {
            for (uint256 c = 0; c < 12; c++) {
                if (pointerExists(tags[t], names[c])) {
                    vm.writeLine(GEN_V4_PATH, v4ImportLine(pointerFile(tags[t], names[c]), bases[c], tags[t]));
                }
            }
        }
