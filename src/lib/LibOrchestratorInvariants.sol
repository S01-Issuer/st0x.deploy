...
/// pinned 0.1.8 address on the active chain.
...
/// @notice The orchestrator beacon does not point at the audited 0.1.8
/// orchestrator implementation.
/// @param expected The pinned 0.1.8 implementation.
...
/// 0.1.8 pin), the beacon is the deployer constructor's first `CREATE`
...
library LibOrchestratorInvariants {
    /// @notice The `UpgradeableBeacon` created by the 0.1.8
    ...
    /// by the FIRST `deploy()` call on the 0.1.8 beacon-set deployer — its
    ...
    /// 0.1.8 beacon-set deployer is live, reports the pinned beacon, and the
    /// beacon points at the audited 0.1.8 orchestrator implementation under
    /// the pinned owner.
    function assertBeaconSet() internal view {
        ...
        if (impl != LibProdDeployV4.ST0X_ORCHESTRATOR_0_1_8) {
            revert OrchestratorBeaconImplMismatch(LibProdDeployV4.ST0X_ORCHESTRATOR_0_1_8, impl);
        }
