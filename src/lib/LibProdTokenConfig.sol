...
/// @notice The canonical name/symbol table for the 20 ST0x production
/// tokens, captured verbatim from the live Base receipt vaults (2026-07-06)
/// so a new chain's token set can be deployed byte-identical to Base. This
/// is the deploy-input companion to `LibTokenInvariants` (which holds the
/// deployed addresses): the deploy script reads this to author the
/// `newTokenAndWrapperVault` calls, and the cross-chain parity pin's
/// name/symbol comparison is against exactly these values (read live off
/// Base at fork time, so a drift between this table and Base fails a test).
///
