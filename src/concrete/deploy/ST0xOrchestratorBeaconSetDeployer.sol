...
///
/// The beacon's implementation is hardcoded from `LibProdDeployCurrent` and
/// its initial owner is resolved from the active chain id — the chain's ST0x
/// token-owner Safe (`LibSafeInvariants.safeForChainId`) — so the constructor
/// takes no dynamic input and this contract is Zoltu-deployable directly: its
/// deterministic address is generated by `script/BuildPointers.sol` like every
/// other deployed contract, and the creation code is identical on every chain
/// while each chain's beacon comes up owned by that chain's Safe. No EOA ever
/// holds beacon upgrade authority and no ownership migration follows the
/// deploy. Deploying on a chain without a pinned Safe reverts in the
/// constructor. Unlike the vault beacon-set deployers, whose parameterised base
...
contract ST0xOrchestratorBeaconSetDeployer is ERC165, IST0xOrchestratorBeaconSetDeployerV1 {
    ...
    constructor() {
        iOrchestratorBeacon = new UpgradeableBeacon(
            LibProdDeployCurrent.ST0X_ORCHESTRATOR, LibSafeInvariants.safeForChainId(block.chainid)
        );
    }
